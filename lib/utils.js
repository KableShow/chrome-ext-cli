...
function openComment (source, sign) {
  // let reg = /\/\*\*ext-template:ts--start(\.+)ext-template:ts--end\*\*\//gi
  let reg = /\/\*\*ext-template:ts--start\n/gi
  let s = source.replace(`/**ext-template:${sign}--start\n`, '')
...
function removeDocumentStart (projectPath) {
  ...
  deleteFile(path.join(projectPath, './static/js/documentStart.bundle.js'))
}
